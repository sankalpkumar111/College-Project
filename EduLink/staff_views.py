...
from .models import Student, AttendanceReport, Subject, Session, Attendance
...
@csrf_exempt
def staff_attendance_percentage(request):
    staff = get_object_or_404(Staff, admin=request.user)
    ...
    if request.method == 'GET':
        ...
        context = {
            'page_title': 'Staff Attendance Percentage',
            'subjects': subjects,
            'sessions': sessions
        }
        
        return render(request, 'staff_template/attendance_percentage.html', context)
        ...
        try:
            ...
            session = get_object_or_404(Session, id=session_id)
            attendance_records = Attendance.objects.filter(subject=subject, session=session)
            ...
            for attendance_record in attendance_records:
                ...
                for student in students:
                    attendance_report = AttendanceReport.objects.filter(student=student, attendance=attendance_record).first()
                ...
                present_count = sum(1 for data in student_data if data["status"] == "Present")
                absent_count = sum(1 for data in student_data if data["status"] == "Absent")
