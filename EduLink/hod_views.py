...
from .models import Staff, NotificationStaff
    ...
    if request.method == 'POST':
        if form.is_valid():
            ...
            try:
                ...
                return redirect(reverse('add_accountant'))
            except Exception as e:
                messages.error(request, f"Could Not Add Accountant: {e}")
        else:
...
def manage_accountant(request):
    accountants = CustomUser.objects.filter(user_type=4)
    context = {
        'accountants': accountants,
        'page_title': 'Manage Accountants'
    }
    return render(request, "hod_template/manage_accountant.html", context)
...
def send_student_notification(request):
    if request.method == 'POST':
        ...
        try:
            ...
            # Send notification via WebSocket or other means
...
def send_staff_notification(request):
    if request.method == 'POST':
        ...
        # Ensure that the 'id' parameter is not empty
        if id is None or id == '':
            return JsonResponse({'success': False, 'error': 'Invalid staff ID'})
        
        # Try to retrieve the staff object by its admin_id
        try:
        ...
        try:
            # Save the notification to the database
            notification = NotificationStaff.objects.create(staff=staff, message=message)
            
            # Send notification via WebSocket
            channel_layer = get_channel_layer()
            async_to_sync(channel_layer.group_send)(
                'staff_notification_group',  # WebSocket group name
                {
